fig5a <- plot_rr_map(out, 1, n_breaks = 100, min_disp_prob=0.4) + ggtitle("Resistant Lineage 1")
...
fig5b <- plot_rr_map(out, 2, n_breaks = 100, min_disp_prob=0.4) + ggtitle("Resistant Lineage 2")
...
y_min <- min(layer_scales(fig5a)$y$range$range, layer_scales(fig5b)$y$range$range)
y_max <- max(layer_scales(fig5a)$y$range$range, layer_scales(fig5b)$y$range$range)

fig5a <- fig5a+ylim(y_min,y_max)
fig5b <- fig5b+ylim(y_min,y_max)

x_min <- min(layer_scales(fig5a)$x$range$range, layer_scales(fig5b)$x$range$range)
x_max <- max(layer_scales(fig5a)$x$range$range, layer_scales(fig5b)$x$range$range)
